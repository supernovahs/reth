...
impl<EF: ExecutorFactory> ExecutionStage<EF> {
    ...
    pub fn execute_inner<DB: Database>(
        ...
        let mut executor = self
            .executor_factory
            .revm_state_with_sp(LatestStateProviderRef::new(provider.tx_ref()))
            .unwrap();
        ...
        let mut bodies_cursor = provider.tx_ref().cursor_read::<tables::BlockBodyIndices>()?;
        ...
        // write reverts
        reverts.write_to_db(provider.tx_ref(), start_block)?;
        // write changes
